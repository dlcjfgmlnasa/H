    ...
    def __init__(self, in_channels: int, pool_sizes: Tuple[int, ...]):
        ...
        for size in pool_sizes:
            self.layers.append(nn.Sequential(
                ...
                nn.GroupNorm(_choose_gn_groups(reduction_channels), reduction_channels),
                nn.SiLU(inplace=True)
            ))
...
class PSPNet1D(nn.Module):
    def __init__(
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.GroupNorm(_choose_gn_groups(ppm_in_channels), ppm_in_channels),
            nn.SiLU(inplace=True),
            nn.Dropout(0.1),
...
if __name__ == "__main__":
    ...
    # 테스트 입력
    dummy_input = torch.randn(BATCH, IN_CH, T_LEN)

    # 모델 실행 및 출력 크기 확인
    output = psp_model(dummy_input)
    print(f"PSPNet Input Shape: {dummy_input.shape}")
