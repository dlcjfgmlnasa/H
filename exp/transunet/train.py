...
from exp.transunet.model import TransUNet1D
from data.utils import get_dataset
from utils.loss import CrossEntropyDiceLoss, DiceLoss
...
def get_args():
    ...
    parser.add_argument('--dataset_name', default='gesture', choices=['heartbeat', 'ahi', 'gesture'])
...
class Trainer(object):
    def __init__(self, args):
        ...
        # Model
        self.model: nn.Module = TransUNet1D(
            length=int(self.train_loader.dataset.fs * self.train_loader.dataset.step_sec),
            segment_size=10,
        ...
        # self.criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor([0.05, 0.2, 0.2, 0.2, 0.2]).to(self.device),
    ...
    def eval_one_epoch(self, epoch: int) -> Dict:
        ...
        result = metric.calculate_segmentation_metrics(y_pred, y_true, num_classes=self.class_num, ignore_index=-1)
